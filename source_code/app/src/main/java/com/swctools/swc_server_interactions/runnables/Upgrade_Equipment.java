...
public class Upgrade_Equipment extends SWC_Runnable_Base implements Runnable {
    private static final String TAG = "Upgrade_Equipment";
    ...
    public Upgrade_Equipment(Handler swc_interaction_handler, String playerId, Context context, String buildingId) {
    ...
    @Override
    public void run() {
        MethodResult methodResult = upgradeEquipment();
    ...
    private MethodResult upgradeEquipment() {
            ...
            try {
                ...
                for (UpgradeItemModel upgradeItemModel : upgradeItemModels()) {
                    ...
                    for (int i = upgradeStart; i <= upgradeItemModel.getMaxLevel(); i++) {
                        ...
                        SWCDefaultResponse swcDefaultResponse = new SWCMessage(playerLoginSession.getAuthKey(), true, playerLoginSession.getLoginTime(), new Cmd_EquipmentUpgrade(
                                playerLoginSession.newRequestId(),
                                equipmentUid,
                                playerId,
                                buildingId
                        ), "upgradeEquipment", context).getSwcMessageResponse();
...
//                            }
    ...
    private ArrayList<UpgradeItemModel> upgradeItemModels() {
        ...
        upgradeItemModels.add(new UpgradeItemModel("eqpRebelATAPHalfSkin",1,10));
        upgradeItemModels.add(new UpgradeItemModel("eqpRebelHovertank",6,10));
